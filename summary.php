		...
		<!--</h6><font color=red>Only current month's reports will be available here</font></h6>-->
			 ...
			 <table class="table table-bordered table-hover table-striped col-xs-12 col-md-6 col-lg-6">
				<tr>
					...
					<td align="center">
						<input type="button" class="btn btn-warning" value="Download" onclick="fnDownloadsummary()">
					</td>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//var startDate = new Date(now.getFullYear() - (now.getMonth() > 0 ? 0 : 1), (now.getMonth() - 3 + 12) % 12, 1);
		...
		oTable = $('#campaignsTable').dataTable( {
			...
			"lengthChange": false,
			"ordering": false,
			"sAjaxSource": "scripts/fetchsummarydetails.php",
	...
	function fetch_data(is_date_search, start_date='', end_date='', campaigntag) {
		var dataTable = $('#campaignsTable').DataTable({
					...
					"order" : [],
					"ajax" : {
						url:"scripts/fetchsummarydetails.php",
						type:"POST",
						data:{
	...
	function fnFilter() {
		...
		oTable.fnReloadAjax('scripts/fetchsummarydetails.php?'+params);
	...
	function fnDownloadsummary(){

		var fromDate = $("#fromdate").val();
                var toDate = $("#todate").val();
                var campaigntag = $.trim($("#campaigntag").val());
		//console.log(fromDate+" "+toDate+" "+campaigntag);
		window.location.href="downloadsummary.php?fromDate="+fromDate+"&toDate="+toDate+"&campaigntag="+campaigntag;
