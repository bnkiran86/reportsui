		...
		if($logId != "" && is_numeric($logId)) {
			...
			if( $PDO_Stmt->rowCount() > 0) {
				...
				if($filename != "" && trim($objUser->fnGetFileExtension($filename)) == "csv") {
					...
					echo "<script>alert('Invalid access');window.location.href='campaignwiselogs.php';</script>";
				}
			...
			echo "<script>alert('Invalid request');window.location.href='campaignwiselogs.php';</script>"; 
		}
